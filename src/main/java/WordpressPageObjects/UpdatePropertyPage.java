				...
				try {
					...
					if (status.equalsIgnoreCase("TO BE UPDATED")) {
						...
						Boolean value = false;
							...
							for (int j = 1; j <= sheet1.getLastRowNum(); j++) {
								...
								if (!cell1.getStringCellValue().equalsIgnoreCase("")) {
									...
									String[] a = propertyLink.split("&preview=true");
									String[] b = a[0].split("&p=");
									propTag = b[1];
									value =  true;
									break;
								}
						...
						if (propertyStatus != null) {
							...
							je.executeScript("arguments[0].scrollIntoView(true);", driver
									.findElement(By.xpath("//label[contains(text(),'" + propertyStatus + "')]/input")));
							wait.until(ExpectedConditions.visibilityOfElementLocated(
							...
							try {
								executor.executeScript("arguments[0].click();", driver.findElement(By.xpath("//label[contains(text(),'" + propertyStatus + "')]/input")));
								}
								catch(Exception e)
								{
									actions.click(driver.findElement(By.xpath("//label[contains(text(),'" + propertyStatus + "')]/input")));
									Action action  = actions.build();
									action.perform();
								}
						...
						wait.until(
								ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[text()='Advanced']")));
						...
						Select select = new Select(driver
								.findElement(By.xpath("//select[contains(@id,'yoast-meta-robots-noindex-metabox')]")));
